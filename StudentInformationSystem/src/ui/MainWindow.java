	...
	private void initialize() throws Exception {
		...
		groupLayout.setHorizontalGroup(groupLayout.createParallelGroup(Alignment.TRAILING)
				.addGroup(groupLayout.createSequentialGroup().addContainerGap()
						.addGroup(groupLayout.createParallelGroup(Alignment.TRAILING)
								.addComponent(tablePanel, Alignment.LEADING, GroupLayout.DEFAULT_SIZE, 740,
										Short.MAX_VALUE)
		...
		groupLayout.setVerticalGroup(groupLayout.createParallelGroup(Alignment.TRAILING)
				.addGroup(groupLayout.createSequentialGroup().addGap(23)
						.addComponent(tablePanel, GroupLayout.DEFAULT_SIZE, 479, Short.MAX_VALUE).addGap(33)
		...
		checkCourseButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (selection == -1) {
					...
					try {
						Student selectedStudent = ((StudentTableModel) table.getModel())
								.valueAtRow(table.convertRowIndexToModel(selection));
						StudentCourseWindow courseWindow = new StudentCourseWindow(selectedStudent, MainWindow.this);
						courseWindow.setVisible(true);
					} catch (Exception ex) {
						System.err.println("Error in creating student course window: " + ex.getMessage());
					}
				}
		...
		editInformationButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (selection == -1) {
					...
					Student selectedStudent = ((StudentTableModel) table.getModel())
							.valueAtRow(table.convertRowIndexToModel(selection));
					EditStudentInfoWindow editWindow = new EditStudentInfoWindow(studentDAO, selectedStudent,
							MainWindow.this);
		...
		deleteEntryButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (selection == -1) {
					...
					Student selectedStudent = ((StudentTableModel) table.getModel())
							.valueAtRow(table.convertRowIndexToModel(selection));
					int confirmationIndex = JOptionPane.showConfirmDialog(frame,
							"确定删除" + selectedStudent.getName() + "？", "删除学生信息", JOptionPane.YES_NO_OPTION,
